      ...
      <svg ref="svgRef" viewBox="0 0 500 500" class="dartboard-svg" @click="handleBoardClick">
        ...
        <g filter="url(#sisalTexture)" transform="translate(250, 250)">
          <path
            ...
            :class="
              'segment ' + (highlightedSegment === `single-${segment}` ? 'segment_highlight' : '')
            "
            @mouseenter="activeSegment = `single-outer-${segment}`"
          ...
          <path
            ...
            :class="
              'segment ' + (highlightedSegment === `double-${segment}` ? 'segment_highlight' : '')
            "
            @mouseenter="activeSegment = `double-${segment}`"
            @mouseleave="activeSegment = null"
          />
          <path
            v-for="(segment, index) in segments"
            ...
            :fill="getDoubleFill(index)"
            :class="
              'segment ' + (highlightedSegment === `triple-${segment}` ? 'segment_highlight' : '')
            "
            @mouseenter="activeSegment = `triple-${segment}`"
            @mouseleave="activeSegment = null"
          />
          <path
            v-for="(segment, index) in segments"
            ...
            :class="
              'segment ' + (highlightedSegment === `single-${segment}` ? 'segment_highlight' : '')
            "
            @mouseenter="activeSegment = `single-inner-${segment}`"
            @mouseleave="activeSegment = null"
          />
          <circle
            cx="0"
        ...
        <g class="dartboard-numbers" pointer-events="none">
          <text
            v-for="(num, i) in dartboardNumbers"
            :key="i"
            :x="250 + 200 * Math.cos((i * 18 - 90) * (Math.PI / 180))"
            :y="250 + 200 * Math.sin((i * 18 - 90) * (Math.PI / 180))"
            text-anchor="middle"
...
const handleBoardClick = (event: MouseEvent) => {
  ...
  send('dart-event', {
    type: 'dart_hit',
    throw: { x: finalX, y: finalY, field: activeSegment.value ?? 'miss' },
  })
}
...
const arcPath = (r1: number, r2: number, start: number, end: number) => {
  const x1 = r1 * Math.cos(start),
    y1 = r1 * Math.sin(start)
  const x2 = r2 * Math.cos(start),
    y2 = r2 * Math.sin(start)
  const x3 = r2 * Math.cos(end),
    y3 = r2 * Math.sin(end)
  const x4 = r1 * Math.cos(end),
    y4 = r1 * Math.sin(end)
  return `M ${x1} ${y1} L ${x2} ${y2} A ${r2} ${r2} 0 0 1 ${x3} ${y3} L ${x4} ${y4} A ${r1} ${r1} 0 0 0 ${x1} ${y1} Z`
}

const getDoublePathD = (i: number) =>
  arcPath(DOUBLE_INNER_R, DOUBLE_OUTER_R, getSegmentAngles(i).start, getSegmentAngles(i).end)
const getTriplePathD = (i: number) =>
  arcPath(TRIPLE_INNER_R, TRIPLE_OUTER_R, getSegmentAngles(i).start, getSegmentAngles(i).end)
const getSingleOuterPathD = (i: number) =>
  arcPath(TRIPLE_OUTER_R, DOUBLE_INNER_R, getSegmentAngles(i).start, getSegmentAngles(i).end)
const getSingleInnerPathD = (i: number) =>
  arcPath(OUTER_BULL_R, TRIPLE_INNER_R, getSegmentAngles(i).start, getSegmentAngles(i).end)
...
const getSegmentInfo = (s: string) => {
  ...
  return {
    name: `${s.includes('double') ? 'DOUBLE' : s.includes('triple') ? 'TRIPLE' : 'SINGLE'} ${num}`,
    score: num * mult,
  }
}
